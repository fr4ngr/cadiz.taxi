...
const startRegex = /onMarkerClick:\s*\(station\)\s*=>\s*\{[\s\S]*?<\/div>`\s*};\s*}/;

const newReturnObj = `onMarkerClick: (station) => {
                    window.mapActionDirections = () => {
                        window.open(\`https://www.google.com/maps/dir/?api=1&destination=\${station.lat},\${station.lon}\`, '_blank');
                    };
                    const eaqiRange = station.aqi <= 20 ? '(0-20)' : (station.aqi <= 40 ? '(21-40)' : (station.aqi <= 60 ? '(41-60)' : (station.aqi <= 100 ? '(61-100)' : '(>100)')));
                    return {
                        ...
                        desc: \`
                            <div style="display: flex; flex-direction: column; gap: 12px; margin-top: 6px; font-family: 'Inter', system-ui, sans-serif;">
                                <div style="display: flex; align-items: flex-start; justify-content: space-between; background: linear-gradient(to right, transparent, \${station.color}15); padding: 12px; border-radius: 8px; border-left: 4px solid \${station.color};">
                                    <div style="width: 100%;">
                                        <div style="font-size: 10px; font-weight: 700; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 2px;">CALIDAD DEL AIRE</div>
                                        <div style="font-size: 20px; font-weight: 800; color: var(--text-primary); letter-spacing: -0.5px; display: flex; align-items: baseline; gap: 6px;">
                                            \${station.status}
                                            <span style="font-size: 14px; color: var(--text-secondary); font-weight: 600;">\${eaqiRange}</span>
                                        </div>
                                        
                                        <div style="display: flex; flex-direction: column; gap: 6px; margin-top: 10px;">
                                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                                <div style="display: flex; flex-direction: column; gap: 1px;">
                                                    <span style="font-size: 10px; font-weight: 700; color: var(--text-secondary); letter-spacing: 0.5px;">ÍNDICE EAQI</span>
                                                    <span style="font-size: 9px; color: var(--text-secondary);">Índice Europeo de Calidad del Aire</span>
                                                </div>
                                                <span style="font-size: 12px; font-weight: 700; color: var(--text-primary);">\${station.aqi} / 110</span>
                                            </div>
