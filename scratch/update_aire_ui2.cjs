...
const startRegex = /const eaqiRange = station.aqi <= 20 \? '\(0-20\)' : \(station.aqi <= 40 \? '\(21-40\)' : \(station.aqi <= 60 \? '\(41-60\)' : \(station.aqi <= 100 \? '\(61-100\)' : '\(>100\)'\)\)\)\);[\s\S]*?<\/div>`\s*};\s*}/;

const newReturnObj = `const eaqiRange = station.aqi <= 20 ? '0-20' : (station.aqi <= 40 ? '21-40' : (station.aqi <= 60 ? '41-60' : (station.aqi <= 100 ? '61-100' : '>100')));
...
if(startRegex.test(code)) {
    ...
    console.log('Successfully replaced HTML structure for Aire popup again!');
} else {
